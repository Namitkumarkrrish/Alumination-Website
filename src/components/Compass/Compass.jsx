...
import { handleCompassMove, resetCompass, snapResetCompass } from "./compassAnimation";

const Compass = ({ compassRef, needleRef }) => {

  useEffect(() => {
    const onMove = (e) => handleCompassMove(e, compassRef.current, needleRef.current);
    const onLeave = () => resetCompass(compassRef.current, needleRef.current);

    // ─── Scroll listener ────────────────────────────────────────────────────
    // When the user scrolls away from the hero section (and then comes back),
    // the tilt transforms from the last mouse position are still baked in.
    // We detect when the compass is no longer in the viewport and hard-reset
    // all 3D transforms so it always appears upright on re-entry.
    const onScroll = () => {
      ...
      if (!isVisible) {
        // Compass is off-screen — snap everything to neutral silently
        snapResetCompass(compassRef.current, needleRef.current);
      }
    };
    ...
    if (heroEl) heroEl.addEventListener("mouseleave", onLeave);

    return () => {
      window.removeEventListener("mousemove", onMove);
      window.removeEventListener("scroll", onScroll);
      if (heroEl) heroEl.removeEventListener("mouseleave", onLeave);
    };
  }, [compassRef, needleRef]);
  ...
  return (
    <div className={styles.compassContainer}>
      ...
      <div ref={compassRef} className={styles.compassWrapper}>
        <svg viewBox="0 0 400 400" className={styles.compassSvg}>
          <defs>
            ...
            <radialGradient id="compassGlow">
              <stop offset="0%" stopColor="rgba(253, 230, 138, 0.2)" />
              <stop offset="40%" stopColor="rgba(180, 83, 9, 0.1)" />
          ...
          <circle cx="200" cy="200" r="200" fill="url(#compassGlow)" opacity="0.5" />
          ...
          <g className={styles.smallDots}>
            {[...Array(24)].map((_, i) => {
              ...
              return <circle key={i} cx={x} cy={y} r="1" fill="#fde68a" opacity="0.5" />;
            })}
          </g>
